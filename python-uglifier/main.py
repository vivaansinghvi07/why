...
def get_true_random_sub(og: str, r: float, tokens: set[str]):
    def get_random_variation(og: str, r: float):
        output = ''
        for c in og:
        ...
        return output
    new_str = get_random_variation(og, r)
...
def mod_imports(lines: list[str], r: int, tokens: set[str]):
    ...
    for idx, line in enumerate(lines):
        if (l:=line.strip().split())[0] in ['import', 'from']:
            ...
            elif kw == "import":  # import ____ as _____
                assert rest[0] == "as"
